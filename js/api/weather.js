...
window.fetchWeather = async function fetchWeather(lat, lon) {
    try {
        ...
        let city = data.name || window.App.data.context.plant?.city || "--";
        _setText("w-city", city);

        _setText("w-temp", `${Math.round(data.main.temp)}°C`);

        _setText("w-wind", `${Math.round(data.wind.speed * 3.6)} km/h`);
...
window.startWeatherRefresh = function startWeatherRefresh() {
    ...
    if (window.App.weatherIntervalId) clearInterval(window.App.weatherIntervalId);
